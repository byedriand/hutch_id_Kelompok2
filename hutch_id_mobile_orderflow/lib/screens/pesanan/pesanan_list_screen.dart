...
class _PesananListScreenState extends State<PesananListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canAddPesanan = userRole == 'staf_penjualan';
    ...
    return Scaffold(
      ...
      body: Consumer<PesananProvider>(
        builder: (context, pesananProvider, _) {
          ...
          final statusFiltered = _selectedStatus.isEmpty
              ? pesananProvider.pesananList
              : pesananProvider.pesananList
                    .where((p) => p.status == _selectedStatus)
                    .toList();

          final pesananList = _searchQuery.isEmpty
              ? statusFiltered
              : statusFiltered.where((p) {
                  final q = _searchQuery;
                  return (p.nomorPo?.toLowerCase().contains(q) ?? false) ||
                      (p.pelanggan?.nama.toLowerCase().contains(q) ?? false);
                }).toList();
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Cari no. PO atau nama pelanggan...',
                ...
                SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final hasImage = item.produk?.fotoUrl != null;
