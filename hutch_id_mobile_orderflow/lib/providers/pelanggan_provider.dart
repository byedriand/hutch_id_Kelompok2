...
class PelangganProvider extends ChangeNotifier {
  ...
  // Start auto-refresh polling (every 10 seconds), silent supaya tidak
  // memunculkan loading penuh layar berulang setiap 10 detik.
  void startAutoRefresh() {
    _refreshTimer?.cancel();
    _refreshTimer = Timer.periodic(const Duration(seconds: 10), (_) {
      fetchPelanggan(silent: true);
    });
  }
  ...
  // Fetch Pelanggan. silent=true untuk polling background.
  Future<void> fetchPelanggan({bool silent = false}) async {
    if (!silent) {
      _isLoading = true;
      _errorMessage = null;
      notifyListeners();
