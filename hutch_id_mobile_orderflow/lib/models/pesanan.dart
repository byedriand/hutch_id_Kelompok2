...
class HistoriStatus {
  ...
  factory HistoriStatus.fromJson(Map<String, dynamic> json) {
    User? userValue;
    if (json['user'] != null && json['user'] is Map) {
      try {
        userValue = User.fromJson(json['user']);
      } catch (e) {
        debugPrint('Error parsing histori user: $e');
      }
    }

    return HistoriStatus(
...
class Pesanan {
  ...
  final User? creator;
  final List<HistoriStatus>? historiStatus;
  ...
  factory Pesanan.fromJson(Map<String, dynamic> json) {
    ...
    // Parse creator (user who created the PO)
    User? creatorValue;
    if (json['creator'] != null && json['creator'] is Map) {
      try {
        creatorValue = User.fromJson(json['creator']);
      } catch (e) {
        debugPrint('Error parsing creator: $e');
      }
    }

    // Parse histori status
    List<HistoriStatus>? historiStatusValue;
    if (json['histori_status'] != null && json['histori_status'] is List) {
      try {
        historiStatusValue = (json['histori_status'] as List)
            .map((item) => HistoriStatus.fromJson(item))
            .toList();
      } catch (e) {
        debugPrint('Error parsing histori_status: $e');
      }
    }
    ...
    return Pesanan(
      ...
      historiStatus: historiStatusValue,
    );
  }
